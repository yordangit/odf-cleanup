...
- `_order_guids_by_complexity()` - Orders **default-RBD-namespace** orphaned GUIDs by cleanup complexity; named-namespace GUIDs are grouped separately by `_group_namespaced_guids()` instead
- `_group_namespaced_guids()` - Orphaned GUIDs living in a named RBD namespace, grouped by namespace - each group is run through `odf-cleanup.py` with `CL_RBD_NAMESPACE` set, then that namespace is removed if now empty
...
- Per named-RBD-namespace GUID (from `_group_namespaced_guids()`), runs the same `odf-cleanup.py`, just with `CL_RBD_NAMESPACE` also set so it opens the right namespace; same failure/logging behavior as the default case. `odf-cleanup.py` itself removes the namespace once it's empty - no separate call needed here
...
#### Decision Mechanisms:
- **Scope Confirmed With User:** `_odf-cleanup.sh` (the auto-triggered prod job) is never triggered for this lab type, so it's left untouched - but `odf-cleanup.py` itself (the repo/dev version) gained `CL_RBD_NAMESPACE` support too, so the generated script can reuse its full pipeline instead of reimplementing a subset of it elsewhere
...
- **Same Tool, Namespace-Scoped:** named-namespace orphaned GUIDs go through the same `odf-cleanup.py` as default-namespace ones (`process_namespaced_guid()` mirrors `process_guid()`), just with `CL_RBD_NAMESPACE` also set - reusing its full discovery/trash-restore/phantom-repair pipeline instead of duplicating a subset of it in the reaper
- **Namespace Itself Is Cleaned Up Too:** `odf-cleanup.py` gained its own `_remove_namespace_if_empty()`, called right before it reports success - so a named-namespace GUID cleanup removes the namespace itself in the same run, no separate script step needed. A named RBD namespace with zero images/trash from the start (`empty_rbd_namespaces`, no GUID ever lived there) still goes through the reaper's namespace-only mode via `remove_empty_namespace()`, since there's nothing for `odf-cleanup.py` to run against

#### Strategy:
- **No Changes to `_odf-cleanup.sh`:** the auto-triggered prod job is never invoked for this lab type, so it's left untouched - `odf-cleanup.py` (repo/dev version) is the one that gained `CL_RBD_NAMESPACE` support and its own namespace-removal logic, same single `ioctx.set_namespace()` call pattern as the reaper already had
- **Reaper Kept Its Own `CL_RBD_NAMESPACE`:** still used for `remove_empty_namespace()` (namespaces with no GUID to clean at all) and `CL_CLEANUP_LIST` (parentless CSI leftovers have no GUID, so `odf-cleanup.py` can't reach them), and as the manual fallback tool when `odf-cleanup.py` fails - same role it already has for default-namespace GUIDs
...
#### **Key Point:**
This gap wasn't a bug in the existing GUID-matching logic - it was a scope gap in *which images the pool's ioctx could even see* in the first place. Fixing it required no new GUID-extraction heuristics (namespaced images use the exact same naming convention), just namespace-aware discovery plus a one-line `set_namespace()` addition to whichever tool needed to reach a given namespace: `odf-cleanup.py` for GUID cleanup (and removing its own now-empty namespace), `odf-descendant-reaper.py` for namespaces with no GUID at all, manual fallback, and GUID-less CSI leftovers.