...
- `_resolve_via_list2()` / `_open_by_ref()` - Fallback for images where `rbd_id`/`rbd_directory` are both missing (not a classic phantom) but `rbd.RBD().list2()` still resolves a real id - opens by that id instead of by name (`OdfImage.open_by_id`), used consistently everywhere an image is reopened (discovery, descendant scanning, flatten, removal). Removal itself still uses the name (confirmed working even when open-by-name fails). See "Unresolvable-by-name image" in `AGENTS.md` for the full corruption-pattern writeup - the mechanism behind why `list2()` succeeds where `rbd_directory` shows nothing is unexplained
...
- **Id-Based Recovery Fallback:** Some images fail open-by-name even though `rbd_id`/`rbd_directory` are both genuinely missing - not classic phantom, but `rbd.RBD().list2()` can still resolve a real id and the image opens/behaves completely normally by id. Handled via `_resolve_via_list2()`/`_open_by_ref()` rather than dropped as unrecoverable
